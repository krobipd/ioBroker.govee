...
describe("DiagnosticsCollector", () => {
    ...
    describe("addMqttPacket", () => {
        it("captures packets with topic + hex", () => {
            ...
            const result = c.generate(makeDevice({ deviceId: "dev1" }), "2.0.0");
            const packets = result.lastMqttPackets as Array<
    ...
    describe("setApiResponse", () => {
        ...
        it("truncates large bodies with marker", () => {
            ...
            const result = c.generate(makeDevice({ deviceId: "dev1" }), "2.0.0");
            const last = result.lastApiResponse as Record<
                string,
                { body: unknown }
            >;
    ...
    describe("forget / clear", () => {
        it("forget(deviceId) drops only that device's buffers", () => {
            ...
            c.forget("dev1");
            expect(
                c.generate(makeDevice({ deviceId: "dev1" }), "2.0.0").recentLogs,
            ).to.deep.equal([]);
            expect(
                (c.generate(makeDevice({ deviceId: "dev2" }), "2.0.0")
                    .recentLogs as Array<unknown>).length,
            ).to.equal(1);
        });
